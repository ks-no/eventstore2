...
public class ProjectionProtobufManager extends UntypedActor {
    ...
    private Map<Class<? extends ProjectionProtobuf>, ActorRef> projections = new HashMap<Class<? extends ProjectionProtobuf>, ActorRef>();
    ...
    public static Props mkProps(ActorRef errorListener, List<Props> props){
        return Props.create(ProjectionProtobufManager.class,errorListener,props);
    }

    public ProjectionProtobufManager(ActorRef errorListener, List<Props> props){
        this.errorListener = errorListener;

        for (Props prop : props) {
            ActorRef projectionRef = getContext().actorOf(prop, prop.actorClass().getSimpleName());
            projections.put((Class<? extends ProjectionProtobuf>) prop.actorClass(), projectionRef);
