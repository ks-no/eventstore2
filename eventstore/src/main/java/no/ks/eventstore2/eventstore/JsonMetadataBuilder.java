...
public class JsonMetadataBuilder {
    ...
    public static Messages.EventWrapper.Builder readMetadataAsWrapper(byte[] metadataBytes) {
        ...
        return Messages.EventWrapper.newBuilder()
                .setAggregateType(metadata.get(AGGREGATE))
                .setAggregateRootId(metadata.get(AGGREGATE_ID))
                .setOccurredOn(Long.parseLong(metadata.get(OCCURRED_ON)))
                .setProtoSerializationType(metadata.get(SERIALIZATION_ID))
                .setCreatedByUser(metadata.get(USER))
                .setJournalid(Long.parseLong(metadata.get(MONGO_JOURNAL_ID)));
    }
}
